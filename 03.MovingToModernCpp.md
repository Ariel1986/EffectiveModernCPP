...
Widget w5{{}}; // ditto
传递两个参数创建std::vector，可以很好的说明用（）和用{}初始化时的不同
...
编译器在类模板Widget中遇到MyAllocList::type（use of the nested typedef)时，不确定它是一个类型还是其他什么
class Wine {...};
...
子类重载基类的函数必须满足如下条件：
基类的函数必须是虚函数（virtual）
基类和子类的函数名必须一致(只有析构函数例外）
基类和子类的函数参数类型必须一致
基类和子类的constness属性必须一致
基类和子类的的返回类型和异常规范（exception specificationn)必须是兼容的
对于C++11，函数的reference qualifiers(引用限定符)必须一致
class Base{
public:
...
相比于non-noexcept函数，noexcept函数更可优化。noexcept函数允许编译器生成更好的代码。 对于如下代码，如果在运行时f抛出了一个异常，对于C++98的异常规范说明，call stack unwound 至f函数的调用者，然后在执行一些与异常无关的代码后，程序执行被终止。对于C++11的异常规范说明，运行时行为有轻微的不同：the stack is only unwound before program execution is terminated.
